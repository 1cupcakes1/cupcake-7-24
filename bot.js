...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('Aleyküm selam, nasılsın? :fire:');
		} else {
...
client.on('warn', e => {
  console.log(chalk.bgYellow(e.replace(regToken, 'that was redacted')));
   ...
   client.on("message", message => {
    const dmchannel = client.channels.find("name", "#🎤søhβε†-øδαsı🎤");
...
client.on("message", async message => {
    ...
    if (command === "ters" || command === "tersyaz") {
        ...
        // Komut kullanıldığında 'h.'  Karakteri ile başla.
        const OFFSET = '!'.charCodeAt(0);
        if (args.length < 1) {
            message.channel.send('Lütfen ters yazcağın yazıyı gir!.');
        }
...
client.on("message", async message => {
 ...
 client.on('message', msg => {
  if (msg.content.toLowerCase() === '+sunucular') {
    ...
    msg.channel.send(`Ben şu an **${bot.guilds.size}** sunucuda **${bot.users.size}** 🐱Beni sunucuna ekle🍭`)
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === prefix + 'bilgi') {
    ...
    .addField(msg.author.username, 'Özel mesajlarını kontrol et. :envelope_with_arrow: :postbox:');
    msg.channel.sendEmbed(ozelmesajkontrol) }
      msg.author.sendMessage('Bot sürümü:🔥Rigby🎮-V2 Yapımcı: :crossed_swords:İlker Retro HD:crossed_swords: **Sohbet ve Oyun**\n\n_**Linkler:**_\n\n**İlker Retro HD Youtube sunucusunun davet linki**:https://discord.gg/warJQwx \n\n**Steam dan eklemek için**:https://steamcommunity.com/profiles/76561198831627614\n\n**:copyright: 2019 İlker Retro HD').then(message => console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Gönderilen mesaj: ${message.content}`)).catch(console.error);
  }
});
